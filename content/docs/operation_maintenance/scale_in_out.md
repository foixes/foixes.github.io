...
集群扩缩容包括两种，一种是节点，即扩容每个 Zone 内的 OBServer 的数量；还有一种是 Zone，即增减 Zone 的数量。

其中，需要在 sys 租户下进行操作。
## 扩容
### 扩容节点
#### 场景
本例从 1-1-1 的集群架构扩容到 2-2-2的集群架构
...
通过系统租户登录到集群，并执行如下命令
```bash
alter system add server 'xxx.xxx.x.xx1:2882' zone 'zone1';
alter system add server 'xxx.xxx.x.xx2:2882' zone 'zone2';
alter system add server 'xxx.xxx.x.xx3:2882' zone 'zone3';
```
...
2. 通过修改租户的 Locality 来增加副本。根据 Locality 的变更规则，每次只能增加一个 Zone 内的 Locality，Locality 的变更规则相关信息请参见 [Locality 概述](https://www.oceanbase.com/docs/common-oceanbase-database-cn-10000000001699416)。
```sql
...
2. 通过修改租户 tenant1 的 Locality 来删除副本。根据 Locality 的变更规则，每次只能删除一个 Zone 内的 Locality，Locality 的变更规则相关信息请参见 [Locality 概述](https://www.oceanbase.com/docs/common-oceanbase-database-cn-10000000001699416)。
```sql
...
```
删除后，可以执行以下语句，确认列表中已查询不到这些 Zone，则表示删除成功。结束后，本次缩容完成。
...
注意：
由于当前版本暂不支持调小租户的 UNIT_NUM，该缩容方式仅适用于当前集群中 Unit 数量小于或等于计划删除 OBserver 节点后的单个 Zone 中的可用 OBserver 数量。例如，本示例中，如果租户 tenant1 的 UNIT_NUM 为 2，则删除各 Zone 中的 OBServer 时会失败。
...
租户资源的扩缩容也是包括两种，一种是增加租户的资源单元（unit）配置大小；另一种是增加租户的资源单元（unit）数量。
...
- 进行租户缩容前，建议进行一轮转储以便释放租户正在使用的内存。手动触发转储的相关操作请参见 [手动触发转储](https://www.oceanbase.com/docs/common-oceanbase-database-cn-10000000001701236)。
- 查看集群中资源的分配情况，了解集群中资源的使用情况。查看集群节点的资源总量和分配状态的相关操作请参见 [查看集群的资源信息](https://www.oceanbase.com/docs/common-oceanbase-database-cn-10000000001701028)。
...
更多 DBA_OB_UNIT_CONFIGS 视图的字段及说明信息请参见 [DBA_OB_UNIT_CONFIGS](https://www.oceanbase.com/docs/common-oceanbase-database-cn-10000000001699272)。
...
注意：调大资源池的 UNIT_NUM 时，修改后的 UNIT_NUM 数量要小于或等于 Zone 内可用的 OBServer 节点数量。
