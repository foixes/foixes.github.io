...
- java 1.8.0+。
- 部署 OceanBase 4.0 版本集群，并创建 3 个租户（flink_cdc_1、flink_cdc_2、flink_cdc_3）。
- 部署 oblogproxy。
- 部署 flink-1.16.1 版本。
- flink-cdc
   - flink-sql-connector-oceanbase-cdc-2.3.0.jar（OceanBase 写入 Flink）
   - flink-connector-jdbc-1.16.1.jar（Flink 写入 OceanBase）
# 软件部署
## 部署 oblogproxy
1、下载 oblogproxy 并解压。
下载地址为：[https://github.com/oceanbase/oblogproxy/releases4](https://github.com/oceanbase/oblogproxy/releases)
...
基于安全考虑，OceanBase 的sys租户账号和密码，需要通过加密配置给 oblogproxy。

执行以下命令即可得到账号和密码的密文，然后将账号和密码的密文分别配置到conf.json中的ob_sys_username和ob_sys_password字段中。

**注意：**

此处的用户必须为sys租户下的用户，若使用非root用户，需要提前在sys租户中创建该用户。
...
# 请输入实际密码
[admin@obtest004 bin]$./logproxy -x Root@2021
8852D*****************A9D8FD52
...
2、编辑flink-conf.yaml配置文件，并根据实际安装路径配置 java 环境变量。
```sql
...
3、下载oceanbase-cdc和jdbc-connector组件。
```bash
# 下载 jar 包
...
从启动结果可以看到主要启动了两个进程standalonesession和askexecutor，其中standalonesession进程对应的是 JobManager 类型，taskexecutor进程对应的是 TaskManager 类型。
...
在租户 flink_cdc_1 中创建数据库并创建表 test1，在 Flink Client 中创建表 products，查看 OceanBase 侧表 test1 的数据能否正常同步至 Flink 侧表 products 中，并在表 test1 中插入一条新数据，在表 products 中查看新插入数据的同步结果。
...
在租户 flink_cdc_2 中创建数据库并创建表 test2，在 Flink Client 中创建表 products ，向表 products 中插入数据，查看在 Flink 侧插入的数据能否正常同步至 OceanBase 侧的表 test2 中 。
...
在 Flink Client 中创建表 flinktest1 和 flinktest2，通过 insert into select 将 cdc@flink_cdc_1 租户的现有数据同步到 cdc@flink_cdc_2 租户中，然后再在上游 OceanBase 侧表 test1 中进行 insert、delete、update，查看下游 OceanBase 表 test2 中数据的变化。
...
在上游的两个租户中分别创建表 flinkcdc1、flinkcdc2 并写入数据，在下游的租户中创建表 flinkcdc3。然后在 Flink Client 中创建表 cdctest1、cdctest2、cdctest3，将表 cdctest1 和 表 cdctest2 进行 join，并将join后的数据写入到表 cdctest3 中。然后分别在上游 OceanBase 侧表 flinkcdc1、flinkcdc2 中进行 insert、delete、update，查看下游 OceanBase 侧表 flinkcdc3 中数据的变化。
...
3、在下游租户的表 flinkcdc3 中查看数据同步，可以看到表 flinkcdc1 和表 flinkcdc2 进行join后的数据已经被同步过来了。
```sql
...
在下游表 flinkcdc3 中查看数据变化，可以看到id=1的一条数据已被删除。
```sql
MySQL [cdc_test3]> select * from flinkcdc3;
+----+------+-------------+--------+---------+--------+
| id | name | description | weight | remarks | kg     |
